...
def start(message):
    ...
    try:
        ...
        # Не отправляем отдельное сообщение об ошибке пользователю
    
    except Exception as e:
        logger.error(f"❌ Ошибка при отправке фото: {e}")
        # Не отправляем отдельное сообщение об ошибке пользователю
    
    # 2. Отправляем ЕДИНОЕ сообщение с текстом и кнопками ПОД фото
    ...
    # Исправленный текст (убрана синтаксическая ошибка)
    welcome_text = (
